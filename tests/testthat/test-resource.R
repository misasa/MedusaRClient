...
test_that("new works with resource_name and connection.",{
  resource_name <- "specimens"
  connection <- Connection$new(list(uri="devel.misasa.okayama-u.ac.jp/Chelyabinsk/", user="admin", password="admin"))
...
test_that("find_all works without parameters.",{
  ...
  connection <- Connection$new(list(uri="devel.misasa.okayama-u.ac.jp/Chelyabinsk/", user="admin", password="admin"))
  obj <- Resource$new(resource_name, connection)
  expect_that(obj$find_all(),shows_message("URI: https://devel.misasa.okayama-u.ac.jp/Chelyabinsk/specimens.json?"))
})
